...
export function arit(description: string, ...args: any[]) {
  ...
  for (let i = 0; i < args.length - 1; i++) {
    ...
    if (test.length !== params.length + 1) {
      throw new Error('Mismatch between the number of row test parameters and the row test function signature. Note that the test signature should include a \'done\' as the last param.');
    }

    it(`${description} #${i + 1} - ${JSON.stringify(params)}`, (done: DoneFn) => {
      params.push(done);
      test.apply(null, params);
    });
